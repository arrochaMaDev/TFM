...
// OBTENER DATOS DE TODAS LAS EVALUACIONES
const evaluacionesRefFromServer: Ref<
  {
    id: number,
    matricula: {
      id: number,
...
const getEvaluacionesData = async () => {
  try {
    ...
    if (!response.ok) {
      ...
      evaluacionesRefFromServer.value = data
      // console.log(data)
      console.table(evaluacionesRefFromServer.value)
    }
  } catch (error) {
...
const evaluacionesByStudentId: Ref<
  {
    id: number,
    matricula: {
      id: number,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (data && data.matriculas && data.matriculas.length > 0 && !visibleDialog.value) {
        evaluacionesByStudentId.value = data;
        console.table(evaluacionesByStudentId.value)
        return data;
      }
...
onMounted(() => {
  fetchData();
  getEvaluacionesData() // obtener datos de todas las matriculas
})
    ...
    table: {
      ...
      style: { 'border': 'none' }
        ...
        <div id="header" class="flex flex-column md:flex-row md:justify-content-between md:align-items-center h-max pt-3 pb-3 border-round-top" style="background-color:  #f8f9fa">
          <h5 class="flex m-0 text-3xl text-800 font-bold pl-1">Listado Evaluaciones</h5>
    ...
    ">
            <ColumnGroup type="header">
              <Row>
                ...
                <Column header="Profesor" :colspan="6" headerClass="h-3rem pl-1"></Column>
              </Row>
              <Row>
            ...
            <Column field="teacher.telefono"></Column>
            <Column field="teacher.email"></Column>
